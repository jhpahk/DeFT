...
# cross-validation을 위해 config.py에 지정된 CV_SPLITS 개의 (train cities, val cities) 조합을 만들어낸다.
...
# original cityscapes dataset과 동일하게 train/val을 나눈다. (not CV)
...
# quality(fine/coarse), mode(train/val/test), cv_split(0 ~ cfg.CV_SPLITS-1)을 주면,
# 해당하는 split에 포함된 item의 list를 만들어 return
...
# train.py 에서 arg.cv의 default 값을 0으로 설정해놓고 따로 실행 script에서 인자로 입력받지 않기 때문에,
# 실질적으로 cv_split 값은 항상 0이고, 결과적으로 cityscapes dataset의 train/val/test 분류를 그대로 따른다.
class CityScapes(data.Dataset):
    ...
    # original image (+ filpped image if flip_bool=True) 를 target size로 resizing하여 return
    ...
    # self.imgs에서 해당하는 index의 img를 불러와 사전에 정의된 여러 transform을 거친 후 return
