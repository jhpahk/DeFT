...
# input에 대해 color augmentation(jittering), blurring을 정의 -> image appearance transformations
# input image에만 적용된다.
# 만약에 dataloader 선에서 frequency filtering 수행하고 싶으면, 여기에 정의하면 될 듯.
...
# color jittering + gaussian blurring / horizontal flip 정의 -> geometric transformations
...
# boundary relaxation 등등...
# 이건 only ground-truth mask에만 적용되는 transformations 인듯?
def get_target_transforms(args, dataset):
    ...
    target_transform = extended_transforms.MaskToTensor()       # MaskToTensor: torch.from_numpy(np.array(img, dtype=np.int32)).long()
...
def setup_loaders(args, mode='main'):
    """
    ...
    if mode == 'pre':
        args.train_batch_size = args.bs_mult_pre * args.ngpu
    ...
    if args.syncbn:
        ...
        if mode == 'pre':
            args.train_batch_size = args.bs_mult_pre
            
        args.val_batch_size = args.bs_mult_val

    args.num_workers = 16 # 4 * args.ngpu
    if args.test_mode:
        args.num_workers = 1
    ...
    if args.syncbn:
        train_sampler = DistributedSampler(train_set, pad=True, permutation=True, consecutive_sample=False)    # permutation=True
    else:
        train_sampler = None

    train_loader = DataLoader(train_set, batch_size=args.train_batch_size,
                              num_workers=args.num_workers, shuffle=(train_sampler is None), drop_last=True, sampler = train_sampler, pin_memory=True)
    ...
    print(f"training dataset: {args.dataset},\n \
            validation dataset: {val_dataset_names},\n \
            extra validation dataset: {args.val_dataset}.\n")
