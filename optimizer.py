...
def get_optimizer(args, net, update='all'):
    """
    ...
    if update == "all":
        ...
        weight_decay = args.weight_decay_enc
    elif update == "dec":
        learning_rate = args.lr_dec
        weight_decay = args.weight_decay_dec
    elif update == "pre":
        learning_rate = args.lr_pre
        weight_decay = args.weight_decay_pre
    elif update == "final":
        learning_rate = args.lr_final
    ...
    if args.sgd:
        ...
        optimizer = optim.AdamW(base_params,
                                lr=learning_rate,
                                weight_decay=weight_decay)
...
def restore_snapshot_overfit(net, optimizer, scheduler, snapshot):
    """
    Restore weights and optimizer (if needed ) for resuming job.
    """
    checkpoint = torch.load(snapshot, map_location=torch.device('cpu'))
